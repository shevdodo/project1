<x-layouts.dashboard title="General Settings">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm max-w-4xl">
        <form method="POST" action="{{ route('superuser.settings.general.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            {{-- Site Icon --}}
            <div>
                <label for="site_icon" class="block text-sm font-semibold text-gray-800 mb-1">Site Icon</label>
                <p class="text-xs text-gray-500 mb-2">Upload an image to be used as your website's main logo.</p>
                <div class="flex items-center space-x-4 mt-2">
                    <div class="w-20 h-20 bg-gray-100 border border-gray-200 rounded-xl flex items-center justify-center text-gray-400 overflow-hidden">
                        @if(isset($settings['site_icon']) && $settings['site_icon'])
                            <img src="{{ asset('storage/' . $settings['site_icon']) }}" alt="Site Icon" class="w-full h-full object-cover">
                        @else
                    ...
                    <div>
                        <input type="file" id="site_icon" name="site_icon" accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100 transition">
                    </div>
                </div>
            </div>
            ...
            {{-- Fav Icon --}}
            <div>
                <label for="fav_icon" class="block text-sm font-semibold text-gray-800 mb-1">Fav Icon</label>
                <p class="text-xs text-gray-500 mb-2">Upload a small square image to be used as your website's favicon.</p>
                <div class="flex items-center space-x-4 mt-2">
                    <div class="w-16 h-16 bg-gray-100 border border-gray-200 rounded-xl flex items-center justify-center text-gray-400 overflow-hidden">
                        @if(isset($settings['fav_icon']) && $settings['fav_icon'])
                            <img src="{{ asset('storage/' . $settings['fav_icon']) }}" alt="Fav Icon" class="w-full h-full object-cover">
                        @else
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                    ...
                    <div>
                        <input type="file" id="fav_icon" name="fav_icon" accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100 transition">
                    </div>
                </div>
            </div>
